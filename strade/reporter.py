"""Non-fatal warning/progress sink and exit-code aggregation.

All output goes to stderr so stdout stays clean for the street list. Both the
...
class Reporter:
    ...
    Warnings increment an error count; a non-zero count yields exit code 1.
    ``verbosity`` gates :meth:`info` (>= 1) and :meth:`debug` (>= 2); other
    methods always emit.
    """
    ...
    def warn(self, message: str) -> None:
        """Record a non-fatal warning and emit it; bumps the exit code to 1."""
        self._error_count += 1
    ...
    def info(self, message: str) -> None:
        """Emit a message when verbosity is >= 1."""
        if self._verbosity >= 1:
            print(f"info: {message}", file=self._stream)

    def debug(self, message: str) -> None:
        """Emit a message when verbosity is >= 2."""
    ...
    def set_counts(
        ...
        """Store counts for :meth:`summary`; only non-``None`` args overwrite."""
    ...
    def summary(self) -> None:
        """Emit the recorded parsed/group/street counts and any error count."""
